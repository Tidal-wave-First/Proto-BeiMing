...
class Wings:
    ...
    def explore(self, task_desc, max_results=5):
        queries = [task_desc]
        captured = 0
        for query in queries:
            ...
            for item in results:
                ...
                if self._ethics_check(text): continue
    ...
    def _local_fallback(self, query):
        if not os.path.exists(self.fallback_source): return []
        with open(self.fallback_source, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        return [{'title':'本地','href':'','snippet':'','full_text':l.strip()} for l in lines if query in l][:3]
